...
def per_data(Sdo, High, maskH,maskS,sc0, lr, ud,K=0.5,sigma=[10,-10,10,-10]):
...
def siftImageAlignment(img1, img2, Hsize, debug=0, mask1=None, mask2=None,KG=0.75,scale=None,san=3,img2Org=None):
    ...
    if img2Org is None: 
        ...
        img2Org,para=array2img(img2Org)
...
#    inlier_idxs=range(len(src))
    ...
    img1Out = warp(img1, tform.inverse, output_shape=(img2.shape[0], img2.shape[1]),order=3)
    img2Out = warp(img2Org, tform, output_shape=(img1.shape[0], img1.shape[1]),order=3)
#
    img3Out=img2array(img2Out,para)
    img2Out = transform.resize(img2Out, (Hsize[0], Hsize[1]),mode='reflect',order=3)
    img2Out=img2array(img2Out,para)

#    img2Out = transform.resize(img2Out, (Hsize[0], Hsize[1]),mode='reflect')
    status = (src[inlier_idxs], dst[inlier_idxs])

    return img1Out, img2Out, img3Out,tform, status,src,dst
...
#    profile = signal.savgol_filter(profile, 11, 3)
    ...
    im2 = im - Ptsetting.convertToCartesianImage(Z)
    im2 = im2 * Disk
    return im2, center, RSUN, Disk
...
def imresize(im,scale,order=None):
    if order is None: order=3
    im2,para=array2img(im)
    im2=rescale(im2,scale,mode='reflect',order=order)
    im2=img2array(im2,para)
    return im2
...
def medianfilter(im,sigma=3):
    from skimage.morphology import disk
    im,para=array2img(im)
    im = filters.median(im, disk(sigma))
    im=img2array(im,para)
    return im